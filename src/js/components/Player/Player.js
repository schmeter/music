    ...
    loadAudio(play = false) {
      ...
      if (file) {
        ...
        if ('mediaSession' in navigator) {
          const src = file.imgPath || config.app.fallbackImage;
          ...
          navigator.mediaSession.metadata = new MediaMetadata({
            ...
            artwork: [
              { src, type, sizes: '96x96' },
              { src, type, sizes: '128x128' },
              { src, type, sizes: '192x192' },
              { src, type, sizes: '256x256' },
              { src, type, sizes: '384x384' },
              { src, type, sizes: '512x512' },
            ],
          });
        }
